...
enemy::enemy(SDL_Renderer *renderer)
{
    ...
    ene_frame.Set_dstRect(mPos_x, mPos_y, OBJ_SIZE_W, OBJ_SIZE_H);
}
...
bool enemy::check_collision_hor(Game_map *map_)
{
    ...
    int right = (mPos_x + OBJ_SIZE_W) / SIZE_TILE;
    int top = mPos_y / SIZE_TILE;
    int bottom = (mPos_y + OBJ_SIZE_H) / SIZE_TILE;

    if (bottom * SIZE_TILE == mPos_y + OBJ_SIZE_H) bottom -= 1;

    if (x_vel > 0)
    {
        if (valid_collision(map_->MapIndex[top][right]) || valid_collision(map_->MapIndex[bottom][right]))
        {
            mPos_x = right * SIZE_TILE - OBJ_SIZE_W;
            return true;
        }
    }
    else if (x_vel < 0)
...
void enemy::ene_move(Game_map *map_)
{
    if (direction == VER_DIRECTION){
        if (check_collision_ver(map_) || (mPos_y <= 0) || (mPos_y + OBJ_SIZE_H >= 14 * SIZE_TILE)) y_vel *= -1;
        mPos_y += y_vel;
    }
    if (direction == HOR_DIRECTION){
        if (check_collision_hor(map_) || ((mPos_x <= 0) || (mPos_x + OBJ_SIZE_W >= 24 * SIZE_TILE))) x_vel *= -1;
        mPos_x += x_vel;
    }
}
